...
struct MacSettingsView: View {
    ...
    private var recordingSections: some View {
        Section("Sessions") {
            ...
            LabeledContent {
                ...
                // no speaker labels, so there is nothing to name — an
                // enabled-but-inert toggle would read as a broken feature (same
                // reasoning as the re-transcription toggle above).
    ...
    private var speakerNamingFootnote: String {
        if isOffline {
            return "Unavailable for an on-device engine — on-device transcription doesn't separate speakers, so there's nothing to name."
        }
