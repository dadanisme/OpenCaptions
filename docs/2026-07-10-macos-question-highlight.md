# macOS transcript question highlighting

**Target:** OpenCaptions (native macOS app)

Adds a "highlight sentences that end in `?`" transcript treatment so questions
stand out at a glance, live and in saved sessions. Detection is purely
string-based — **no** `TranscriptionLine` change and **no** model flag.
...
- **`OpenCaptions/Views/SessionDetail/HighlightedMessageText+Caching.swift`** — the sentence scanner
  (`findQuestionRanges`), `buildSegments`, `buildAttributedString`,
...
### 1. Question-highlight only — no name highlighting
A `HighlightedMessageText` could do two things: highlight the signed-in user's
...
transcript UI, so name highlighting would have no input and no consumer. It is
omitted entirely: no `findNameRanges`, no regex cache, no `.nameHighlight` segment
type. The function set is just `findQuestionRanges`, `buildSegments`, and
`buildAttributedString`.

### 2. Font & foreground are inherited, not set on the runs
Rather than set an explicit font and a per-segment foreground on every
`AttributedString` run, macOS leaves the runs' font/foreground **unset**, so the
surrounding view's `.font(.transcript(...))`
...
### 3. `buildSegments` uses merge-then-interleave
With only one highlight dimension, `buildSegments`
...
the output never duplicates, drops, or reorders text. In that nested case the merge
highlights the whole quoted span.
...
- A fixed pale amber (`0xFFF3CD`) — too subtle on the Mac.
...
The whole point of the feature is *at-a-glance*
...
never touch the cache, so only stable committed lines populate it. Surfaced by the
adversarial review.
