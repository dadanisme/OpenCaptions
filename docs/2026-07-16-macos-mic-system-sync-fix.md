...
(`OpenCaptionsAEC.mm:104-105`), the wrong direction, and is called with `0` anyway.
...
- **Option C — near-end mic delay.** If measurement shows the reference *still* lags
  ...
  or a symmetric near-end knob on `OpenCaptionsAEC`).
...
- `OpenCaptions/Services/Audio/AudioRingBuffer.swift` — add `trim(toMaxAvailable:)`.
- `OpenCaptions/Services/Audio/MixedAudioCaptureService.swift` — `systemRefCushionSamples`
  constant + mix-stat counters.
- `OpenCaptions/Services/Audio/MixedAudioCaptureService+Mic.swift` — trim before read;
  record stats.
- `OpenCaptions/Services/Audio/MixedAudioCaptureService+Stats.swift` — `recordMixStats`.

## Verification

Build the **OpenCaptions** scheme in Xcode. Source = **Microphone + System Audio**, on
...
the `OpenCaptionsAEC ready` line confirms it built). Read the logged `n` and occupancy to
